...
@app.route('/index')
def index():
    return "Hello, World - Nginx! bdb-{} ".format(bdb)
...
def drawGraph(sensor_id):
    points = []
    for point in collection.find({"data.entity": "reading","data.resource_id":sensor_id}).limit(1000).sort('data.Date', DESCENDING):
        graphPoint={}
        try:
